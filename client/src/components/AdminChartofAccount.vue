    ...
    <v-layout>
      <v-flex sm12 md10 offset-md1>
        <v-card outlined class="my-6">
          <v-card-title>
            <v-col cols="12" md="3">
              <v-radio-group dense class="mt-4" v-model="radio">
            ...
            <v-col cols="12" md="1">
              <v-btn v-if="radio==='active'" :hidden="!hasSelected" fab small dark color="orange darken-2" @click="deactivateDialog = true">
                <v-icon>clear</v-icon>
              </v-btn>
              <v-btn v-else-if="radio==='inactive'" :hidden="!hasSelected" fab small dark color="success" @click="activateAccount">
                <v-icon>check</v-icon>
              </v-btn>
            </v-col>
            <v-col cols="12" md="1">
              <v-btn :hidden="!hasSelected" fab small color="error" @click="deleteDialog = true">
          ...
          >
            <v-card>
              ...
              <v-card-actions>
                <div class="flex-grow-1"></div>
                <v-btn color="blue darken-1" text @click="deactivateDialog = false">Cancel</v-btn>
...
export default {
  ...
  methods: {
    ...
    async deactivateAccount () {
      ...
      try {
        await axios.put('/api/accounts/inactive', items, config)
        this.update++
    ...
    async activateAccount () {
      ...
      try {
        await axios.put('/api/accounts/active', items, config)
        this.update++
        this.selected = []
      } catch (e) {
        this.getError(e.response.data)
  ...
  computed: {
    ...
    // Filter Accounts if active or inactive
    filterAccounts () {
      if (this.radio === 'active') {
        return this.accounts.filter(account => account.status === 'active')
      } else {
        return this.accounts.filter(account => account.status === 'inactive')
      }
    }
  },
  async mounted () {
    ...
    try {
      this.loading = true
      const response = await axios.get('/api/charts/all', config)
