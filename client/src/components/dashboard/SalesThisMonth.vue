  ...
  <div>
    ...
    >
      ...
      <v-card-text>
        ...
        <p v-else class="text-center headline font-weight-medium">&#8369;{{formatSales}}</p>
      </v-card-text>
    </v-card>
  </div>
</template>
...
export default {
  ...
  async mounted () {
    try {
      this.loading = true
      const response = await axios.get(`/api/dashboard/sales/${this.auth.user.BranchId}/${this.formatDate}`)
